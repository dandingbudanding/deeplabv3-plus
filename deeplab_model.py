...
def atrous_spatial_pyramid_pooling(inputs, output_stride, batch_norm_decay, is_training, depth=256):
 ...
 ## W matrix
def conv_w(net, scale=8, num_class=5, pad=0):
    return layers_lib.conv2d(net, num_class * scale * scale, [1, 1])

## P matrix
def conv_p(net, scale=8, num_class=5, pad=0):
    return layers_lib.conv2d(num_class * scale * scale, net, kernel_size=1, padding=pad)

#TensorFlow有两种数据格式NHWC和NCHW，默认的数据格式是NHWC
def DUpsampling(net, scale=8, num_class=5, pad=0):
    x = conv_w(net=net, scale=scale, num_class=5)
    #N, C, H, W = x.size()
    #N, H, W, C =x.get_shape()[0].value,x.get_shape()[1].value,x.get_shape()[2].value,x.get_shape()[4].value
    N, H, W, C = -1,90,90,320#tf.shape(x)  #1,129,129,1024
    # N, W, H, C
    # x_permuted = x.permute(0, 3, 2, 1)

    x_permuted = tf.transpose(x, [0, 2,1,3])

    # N, W, H*scale, C/scale
    x_permuted = tf.reshape(x_permuted,(N, W, H * scale, int(C / scale)))

    # N, H*scale, W, C/scale
    # x_permuted = x_permuted.permute(0, 2, 1, 3)
    x_permuted = tf.transpose(x_permuted, [0, 2, 1, 3])
    # N, H*scale, W*scale, C/(scale**2)
    # x_permuted = x_permuted.contiguous().view(
    #     (N, W * self.scale, H * self.scale, int(C / (self.scale * self.scale))))
    x_permuted = tf.reshape(x_permuted,(N, H * scale, W * scale, int(C / (scale * scale))))
    # N, C/(scale**2), H*scale, W*scale
    # x = x_permuted.permute(0, 3, 1, 2)
    #x = tf.transpose(x_permuted, [0, 2, 1, 3])
      ...
      with tf.contrib.slim.arg_scope(resnet_v2.resnet_arg_scope(batch_norm_decay=batch_norm_decay)):
        with arg_scope([layers.batch_norm], is_training=is_training):
          ...
          with tf.variable_scope("upsampling_logits"):
            #low_level_features = tf.image.resize_bilinear(low_level_features, tf.shape(net)[1:3], name='upsample_1')
            ...
            #logits = layers_lib.conv2d(net, 64, [1, 1], activation_fn=None, normalizer_fn=None,scope='conv_1x1_1')
            #logits = DUpsampling(logits, scale=output_stride, num_class=5, pad=0)  # 1,2064,2064,4
            #logits = tf.image.resize_bilinear(logits, inputs_size, name='upsample_2')
...
import lovasz_losses_tf as love
def deeplabv3_plus_model_fn(features, labels, mode, params):
  ...
  valid_logits = tf.dynamic_partition(logits_by_num_classes, valid_indices, num_partitions=2)[1]  #预测结果
  valid_labels = tf.dynamic_partition(labels_flat, valid_indices, num_partitions=2)[1]  #标签
  ...
  scaled_labels = tf.reshape(valid_labels, shape=[-1])
  one_hot_labels = slim.one_hot_encoding(scaled_labels, params['num_classes'], on_value=1.0, off_value=0.0)
  ignore_label=0
  not_ignore_mask = tf.to_float(tf.equal(scaled_labels,1)) * label0_weight + \
                    tf.to_float(tf.equal(scaled_labels,2)) * label1_weight + \
                    tf.to_float(tf.equal(scaled_labels,3)) * label2_weight + \
                    tf.to_float(tf.equal(scaled_labels,4)) * label3_weight + \
                    tf.to_float(tf.equal(scaled_labels,ignore_label)) * ignore_weight
  cross_entropy = tf.losses.softmax_cross_entropy(one_hot_labels, tf.reshape(valid_logits, shape=[-1, params['num_classes']]),weights=not_ignore_mask)
  ...
  #   Facalloss=focalloss.focal_loss(4, gamma=2., alpha=.25, e=0.1)
  #   loss=Facalloss(valid_labels, valid_logits)
     alogit=logits
     alabel= tf.reshape(labels, [-1,_HEIGHT,_WIDTH])
     loss=love.lovasz_softmax(alogit, alabel,classes='present',ignore=None, per_image=False,order='BHWC')
  if mode == tf.estimator.ModeKeys.TRAIN:
    ...
    elif params['learning_rate_policy'] == 'cos':
        # 余弦衰减
        learning_rate = tf.train.cosine_decay(
            params['initial_learning_rate'], global_step=global_step, decay_steps=params['initial_global_step'])
    else:
    ...
    #optimizer=tf.train.AdamOptimizer(learning_rate=learning_rate)#201907170909改
