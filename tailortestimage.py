    ...
    for i in range(0, imageNum_row):
        for j in range(0, imageNum_col):
            if j == imageNum_col - 1 and i != imageNum_row - 1:
                ...
                num += 1
                cv2.imwrite(savefilepath + ("\\%d.bmp" % (num)), img)
                print("save image success, %d.jpg" % num)
            elif i == imageNum_row - 1 and j == imageNum_col - 1:
                r = np.zeros((rowNum, colNum))
                g = np.zeros((rowNum, colNum))
...
if __name__ == '__main__':
    ...
    block_XSize = 512
    block_YSize = 512
    FolderPath = os.path.abspath('..') + "/data/test/jingwei_round2_test_a_20190726/"
    ...
    for i in file_list:
        ...
        if dataset is None:
            ...
            savefilepath = FolderPath + "tailor/%s" %(i[:-4])
            ...
            for t in threads:
                t.join()
            if (row - threadNum*thread_blockNum*block_YSize):
